...
async def health_check():
    return {"status": "alive", "port": 6699, "service": "RUNPD Standalone"}

def run_standalone_server():
    """Run standalone server on port 6699"""
    uvicorn.run(standalone_app, host="0.0.0.0", port=6699, log_level="info")

def start_backup_server():
    """Start server in background thread"""
    thread = Thread(target=run_standalone_server, daemon=True)
    thread.start()
    print("🚀 RUNPD Standalone API started on http://0.0.0.0:6699")
